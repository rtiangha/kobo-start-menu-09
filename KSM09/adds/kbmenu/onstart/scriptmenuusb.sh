...
while [ "$menu" != "EXIT" ]; do
  ...
  if [ $MODULE_LOADED -gt 0 ]; then
    statusinfo="USB support: enabled"
    which_module="usb"
  else
    MODULE_LOADED=`lsmod | grep -c g_ether`
    if [ $MODULE_LOADED -gt 0 ]; then
      statusinfo="USBNet support: enabled"
      [ -f /var/run/dropbear.pid ] && statusinfo="$statusinfo (plus dropbear)"
      which_module="usbnet"
    else
      statusinfo="USB support: disabled<br />USBNet support: disabled"
    fi
  fi
  setoptions
